...
export async function POST(req: Request) {
    try {
        const { itemName, purchaseCategory, supplierName, currentQuote, lastPrice, targetPrice, annualQuantity, polymerGrade, lmeRate, efficiencyClass } = await req.json();
...
Please provide thorough, accurate market intelligence. If you are not sure about something, clearly state that. Do not fabricate any details.`;
        ...
        return NextResponse.json({ result: aiResult.choices[0].message.content });
    } catch (error: any) {
